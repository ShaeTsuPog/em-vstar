...
void HandleAction_ActionFinished(void)
{
    ...
    *(gBattleStruct->monToSwitchIntoId + gBattlerByTurnOrder[gCurrentTurnActionNumber]) = gSelectedMonPartyId = PARTY_SIZE;
    ...
    gHitMarker &= ~(HITMARKER_DESTINYBOND | HITMARKER_IGNORE_SUBSTITUTE | HITMARKER_ATTACKSTRING_PRINTED
                    | HITMARKER_NO_PPDEDUCT | HITMARKER_IGNORE_SAFEGUARD | HITMARKER_PASSIVE_DAMAGE
                    | HITMARKER_OBEYS | HITMARKER_WAKE_UP_CLEAR | HITMARKER_SYNCHRONISE_EFFECT
                    | HITMARKER_CHARGING | HITMARKER_NEVER_SET | HITMARKER_IGNORE_DISGUISE | HITMARKER_STRING_PRINTED);
            ...
            case ABILITY_JUICE_MAKER:
                if (gBattleMons[battler].hp < gBattleMons[battler].maxHP && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    ...
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    BattleScriptPushCursorAndCallback(BattleScript_JuiceMakerActivates);
                    effect++;
                }
                break;
            case ABILITY_MOODY:
...
/*             SOLAR_POWER_HP_DROP:
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_GRAPPLE_JAW:
            ...
            break;
        case ABILITY_MYTHIC_SOUL:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerTarget].hp != 0
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && CanBeBurned(gBattlerTarget)
             && TARGET_TURN_DAMAGED
             && (gBattleMoves[gCurrentMove].type == TYPE_DRAGON)
             && (Random() % 3) == 0)
            {
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    case ABILITY_GALVANIZE:
        if (moveType == TYPE_ELECTRIC && gBattleStruct->ateBoost[battlerAtk])
            MulModifier(&modifier, UQ_4_12(1.2));
        break;
    case ABILITY_VULKANIZE:
        if (gBattleStruct->ateBoost[battlerAtk])
            MulModifier(&modifier, UQ_4_12(1.2));
        break;
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_NORMAL || TYPE_FIGHTING) && gBattleMoves[gCurrentMove].flags & FLAG_SOUND
    && GetBattlerAbility(battlerAtk) == ABILITY_CACOPHONY && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
     ...
     && GetBattlerAbility(battlerAtk) == ABILITY_MYTHIC_SOUL
     && mod == UQ_4_12(0.0))
     {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_MYTHIC_SOUL);
     }
