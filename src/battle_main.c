...
EWRAM_DATA u8 gDisplayedShinyStringBattle[400] = {0};
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
    {
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 12))))
            {
                ...
                /*if (gBattleTypeFlags & BATTLE_TYPE_TRAINER*/
                if (!(gBattleTypeFlags & BATTLE_TYPE_TRAINER) 
                         && gBattleResources->bufferB[gActiveBattler][1] == B_ACTION_AUTO_RUN)
                {
                        gSelectionBattleScripts[gActiveBattler] = BattleScript_AutoRun;
...
static void TurnValuesCleanUp(bool8 var0)
{
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
    {
        ...
        gSpecialStatuses[gActiveBattler].triplicateState = TRIPLICATE_OFF;
    }
...
void LoadTypeIcon(u8 type)
{
    if (gBattleMoveTypeSpriteId == MAX_SPRITES)
    {
        ...
        gBattleMoveTypeSpriteId = CreateSprite(&gSpriteTemplate_MoveTypes, 216, 145, 0);
        ...
        || gStatuses4[battlerAtk] & STATUS4_ELECTRIFIED)
    {
        gBattleStruct->dynamicMoveType = TYPE_ELECTRIC | F_DYNAMIC_TYPE_2;
             ...
             && ((attackerAbility == ABILITY_PIXILATE && (ateType = TYPE_FAIRY))
                 ...
                 || ((attackerAbility == ABILITY_SHADIFY) && (ateType = TYPE_DARK))
                )
             )
    {
